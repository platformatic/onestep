...
test('query', async () => {
    const [{ foo }] = await db.query((0, sql_1.default) `SELECT 1 + 1 as foo`);
    expect(foo).toBe(2);
});
test('query - multiple queries', async () => {
    const resultA = await db.query([(0, sql_1.default) `SELECT 1 + 1 as foo`]);
    expect(resultA).toEqual([[{ foo: 2 }]]);
    const resultB = await db.query([
        (0, sql_1.default) `SELECT ${1} + 1 as foo;`,
        (0, sql_1.default) `SELECT 1 + ${2} as bar;`,
    ]);
    expect(resultB).toEqual([[{ foo: 2 }], [{ bar: 3 }]]);
});
test('query with params', async () => {
    const [{ foo }] = await db.query((0, sql_1.default) `SELECT 1 + ${41} as ${sql_1.default.ident('foo')}`);
    expect(foo).toBe(42);
});
test('json', async () => {
    await db.query((0, sql_1.default) `CREATE SCHEMA json_test`);
    await db.query((0, sql_1.default) `CREATE TABLE json_test.json (id TEXT NOT NULL PRIMARY KEY, val JSONB NOT NULL);`);
    async function setOneImpliedJson(id, val) {
        const [result] = await db.query((0, sql_1.default) `
    ...
    async function setOneExplicitJson(id, val) {
        const [result] = await db.query((0, sql_1.default) `
      INSERT INTO json_test.json (id, val) VALUES (${id}, ${JSON.stringify(val)})
      ON CONFLICT (id) DO UPDATE SET val=EXCLUDED.val
      RETURNING *;
    `);
        return result;
    ...
    async function setMany(...values) {
        const results = await db.query(values.map(({ id, val }) => (0, sql_1.default) `
    ...
    await expect(setMany({ id: 'obj', val: { foo: 'bar' } }, { id: 'arr', val: ['my string', 56] }, { id: 'str', val: 'my string' }, { id: 'num', val: 42 })).resolves.toEqual([
        { id: 'obj', val: { foo: 'bar' } },
...
test('bigint', async () => {
    await db.query((0, sql_1.default) `CREATE SCHEMA bigint_test`);
    await db.query((0, sql_1.default) `CREATE TABLE bigint_test.bigints (id BIGINT NOT NULL PRIMARY KEY);`);
    await db.query((0, sql_1.default) `
    ...
    VALUES (1),
           ...
           (42);
  `);
    const result = await db.query((0, sql_1.default) `SELECT id from bigint_test.bigints;`);
    expect(result).toEqual([{ id: 1 }, { id: 2 }, { id: 42 }]);
    expect(await db.query((0, sql_1.default) `SELECT id from bigint_test.bigints;`)).toEqual([
        { id: 1 },
        { id: 2 },
        { id: 42 },
    ]);
