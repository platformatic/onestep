...
exports.toTypeName = exports.toValidIdentifier = exports.checkInvalidCharacter = void 0;
const typescript_1 = require("typescript");
function checkInvalidCharacter(text, target) {
...
exports.checkInvalidCharacter = checkInvalidCharacter;
function toValidIdentifier(text, target) {
    if (/^\d/.test(text)) {
        text = '$' + text;
    }
    let res = '';
...
function toTypeName(text, target) {
    if (/^\d/.test(text)) {
        text = '$' + text;
    }
    let res = '';
    ...
    for (let i = 1; i < text.length; i++) {
        const ch = text.charCodeAt(i);
        const isValid = ch !== 95 && isIdentifierPart(ch, target);
        if (isValid) {
            let char = String.fromCharCode(ch);
            if (!lastCharWasValid) {
                char = char.toUpperCase();
        ...
        lastCharWasValid = isValid && ch !== 36;
    }
    return res || '_';
}
...
function isUnicodeIdentifierStart(code, languageVersion) {
    return languageVersion >= typescript_1.ScriptTarget.ES2015
        ? lookupInUnicodeMap(code, unicodeESNextIdentifierStart)
        : languageVersion === typescript_1.ScriptTarget.ES5
