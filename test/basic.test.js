...
test('action should fail if action is called by unsupported event', async (t) => {
  try {
    ...
    const lastLine = err.stdout.split('\n').pop()
    t.equal(lastLine, '::error::The action only works on push, pull_request and workflow_dispatch events')
  }
})

test('action should successfully deploy platformatic project from workflow_dispatch event', async (t) => {
  t.plan(15)
...
test('action should successfully deploy platformatic project from workflow_dispatch event with label', async (t) => {
  t.plan(15)
